...
| 0.4.0 Release compile | Passed with zero warnings/errors. |
| 0.4.0 package structure | Must match the final recorded artifact in `DEVELOPMENT_AND_RELEASE.md`. |
| 0.4.0 live runtime | Pending the user's next run. |
...
## Focused 0.4.0 live sequence

1. Confirm `Elin's PEAK 0.4.0 loaded` and healthy hook lines for Afflictions, Petrification, Backpack Inventory, Backpack Wheel, and Backpack Visuals.
2. Confirm ESC layout: Main and blue Resiliency on the left, no green Recovery panel, two buttons on the right.
3. Use +10 once and reset once. All 18 visible skills should move; no retired Recovery row/key should reappear.
...
7. Gain Petrification from an amulet and a Citadel surface/collision. Confirm each positive gain awards Petrification Resistance once; clearing Petrification awards nothing.
...
- `NaturalRecovery:<Status>` may receive a speed multiplier but never XP in 0.4.0.
